...
subplot(2,1,1)
velocityPlot = plot(data_t,NaN(numRidges,wf));
title("Velocity vs Time plot"); xlabel('Time [s]'); ylabel('Velocity [m/s]');

subplot(2,1,2)
rangePlot = plot(data_t,NaN(numRidges,wf));
title("Range vs Time plot"); xlabel('Time [s]'); ylabel('Range [m]');

figure('Units', 'normalized', 'OuterPosition', [0 0 0.4 0.4])
sp = imagesc(data_d, data_t, spec, [-10 0]); colorbar;
title('range vs time spectrogram');
xlabel('range [m]'); ylabel('t+ time [s]');
...
while i < (size(data, 1) - 4*N)
    ...
    sfft_tmp = fft(sweep_mti(1:N), padding*N);
    sfft_up(1,:) = sfft_tmp(1:max(range_sps,velocity_sps));
    sfft_tmp = fft(sweep_mti(N+1:end), padding*N);
    sfft_dn(1,:) = sfft_tmp(1:max(range_sps,velocity_sps));

    % find fridges
    [fridge_up, ~, ~] = tfridge(rot90(sfft_up(:, 1:max(range_sps,velocity_sps))), chrip_f, 1,'NumRidges',numRidges);
    [fridge_dn, ~, ~] = tfridge(rot90(sfft_dn(:, 1:max(range_sps,velocity_sps))), chrip_f, 1,'NumRidges',numRidges);
    ...
    set(sp, 'CData', spec);
    set(rangePlot, {'YData'}, num2cell(rng',2));
    set(velocityPlot, {'YData'}, num2cell(vel',2));
